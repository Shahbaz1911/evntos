...
const eventFormSchema = z.object({
  ...
  imageUrl: z.string().url("A valid image URL is required after upload.").optional().or(z.literal('')),
...
export default function EventForm({ event, onSubmit, isSubmitting, isGeneratingSlug }: EventFormProps) {
  ...
  const { register, handleSubmit, formState: { errors }, watch, setValue, trigger } = useForm<EventFormValues>({
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)}>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <ImageKitUploader 
            initialImageUrl={watch("imageUrl")}
            onUploadSuccess={(url) => {
